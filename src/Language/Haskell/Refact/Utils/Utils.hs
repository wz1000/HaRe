...
hscFrontend ref mod_summary = do
    ...
    if keepInfo
      then do
        ...
        hsc_env1 <- GHC.getHscEnv
        (tc_gbl_env,rn_info) <- liftIO $ GHC.hscTypecheckRename hsc_env1 mod_summary hpm

        details <- liftIO $ GHC.makeSimpleDetails hsc_env1 tc_gbl_env
